#!/bin/bash

#SBATCH --job-name=rap_metric_caching_navhard
#SBATCH --output=/fs/nexus-projects/sim2real/aliu/RAP/my_dump/%x.out.%j
#SBATCH --error=/fs/nexus-projects/sim2real/aliu/RAP/my_dump/%x.out.%j

## Scale ntasks with gpus
#SBATCH --mem=120gb
#SBATCH --gres=gpu:rtxa5000:1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=32

## GAMMA training config
#SBATCH --time=48:00:00
#SBATCH --qos=huge-long
#SBATCH --account=gamma
#SBATCH --partition=gamma

eval "$(conda shell.bash hook)"
conda activate rap

export NAVSIM_DEVKIT_ROOT="/fs/nexus-projects/sim2real/aliu/RAP"
export NAVSIM_EXP_ROOT="/fs/nexus-projects/sim2real/aliu/RAP/exp"
export OPENSCENE_DATA_ROOT="/fs/nexus-projects/sim2real/aliu/navsim/dataset"

export NUPLAN_MAPS_ROOT="/fs/nexus-projects/sim2real/aliu/navsim/dataset/maps"
export NUPLAN_MAP_VERSION="nuplan-maps-v1.0"
TRAIN_TEST_SPLIT=navhard_test_e2e
CACHE_PATH=/fs/nexus-projects/sim2real/aliu/RAP/metric_cache_navhard

python $NAVSIM_DEVKIT_ROOT/navsim/planning/script/run_metric_caching.py \
train_test_split=$TRAIN_TEST_SPLIT \
cache.cache_path=$CACHE_PATH \
experiment_name=rap_metric_caching_navhard
