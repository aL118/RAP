...
class GroundingSAM:
    ...
    def infer(self, image_path: str, categories: List[str], box_threshold: float = 0.3, text_threshold: float = 0.25) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if len(boxes) > 0:
            ...
            # An empty (0, 1, H, W) tensor, not None -- the docstring above promises a
            # tensor of shape (n, 1, H, W), and n == 0 is a normal outcome on a frame the
            # detector finds nothing in. Returning None made callers that zip the four
            # return values die on that frame instead of skipping it
            # (infer_frames.py:62, job 7420320, frame 19 of 65).
            masks = torch.zeros(
                (0, 1, H, W), dtype=torch.bool, device=transformed_boxes.device)
